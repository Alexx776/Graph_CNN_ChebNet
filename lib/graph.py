...
def distance_lshforest(z, k=4, metric='cosine'):
    """Return an approximation of the k-nearest cosine distances using RAPIDS cuML."""
    assert metric == 'cosine'
    
    # Convertir les données en GPU array si ce n'est pas déjà le cas
    if not isinstance(z, cp.ndarray):
        z = cp.array(z)
    
    # Initialiser NearestNeighbors de cuML
    nn = NearestNeighbors(n_neighbors=k+1, metric=metric)
    nn.fit(z)
    
    # Utiliser tqdm pour afficher la progression
    dist, idx = [], []
    for i in tqdm(range(z.shape[0]), desc="Calculating distances"):
        d, idx_i = nn.kneighbors(z[i:i+1], n_neighbors=k+1)
        
        # Convertir les résultats en numpy arrays pour la compatibilité
        d = cp.asnumpy(d)
