...
class base_model(object):
    ...
    def fit(self, train_data, train_labels, val_data, val_labels):
        ...
        for step in range(1, num_steps + 1):
            ...
            if isinstance(batch_data, np.ndarray) is False:
            ...
            if step % self.eval_frequency == 0 or step == num_steps:
                ...
                summary = sess.run(self.op_summary, feed_dict)
                summary = tf.compat.v1.Summary(value=[
                    tf.compat.v1.Summary.Value(tag='validation/accuracy', simple_value=accuracy),
                    tf.compat.v1.Summary.Value(tag='validation/f1', simple_value=f1),
                    tf.compat.v1.Summary.Value(tag='validation/loss', simple_value=loss)
                ])
    ...
    def loss(self, logits, labels, regularization):
        ...
        with tf.compat.v1.name_scope('loss'):
            ...
            # Summaries for TensorBoard.
            tf.compat.v1.summary.scalar('loss/cross_entropy', cross_entropy)
            ...
            with tf.compat.v1.name_scope('averages'):
                ...
                tf.compat.v1.summary.scalar('loss/avg/cross_entropy', averages.average(cross_entropy))
                tf.compat.v1.summary.scalar('loss/avg/regularization', averages.average(regularization))
                tf.compat.v1.summary.scalar('loss/avg/total', averages.average(loss))
...
class fgcnn2(base_model):
    ...
    def _inference(self, x, dropout):
        ...
        with tf.compat.v1.name_scope('gconv1'):
            # Transform to Fourier domain
...
class cgcnn2_4(base_model):
    ...
    def _inference(self, x, dropout):
        with tf.compat.v1.name_scope('gconv1'):
            ...
            y = filter(xt0, 0)
            if self.K > 1:
                xt1 = tf.sparse.sparse_dense_matmul(self.L, xt0)
                y += filter(xt1, 1)
            for k in range(2, self.K):
                xt2 = 2 * tf.sparse.sparse_dense_matmul(self.L, xt1) - xt0  # M x N
                y += filter(xt2, k)
                xt0, xt1 = xt1, xt2
            # Bias and non-linearity
#            b = self._bias_variable([1, 1, self.F])
...
class cgcnn(base_model):
    ...
    def fourier(self, x, L, Fout, K):
        ...
        # Fourier basis
        _, U = graph.fourier(L)
        U = tf.compat.v1.constant(U.T, dtype=tf.float32)
        # Weights
    ...
    def spline(self, x, L, Fout, K):
        ...
        U = tf.compat.v1.constant(U.T, dtype=tf.float32)  # M x M
        # Spline basis
        B = bspline_basis(K, lamb, degree=3)  # M x K
        #B = bspline_basis(K, len(lamb), degree=3)  # M x K
        B = tf.compat.v1.constant(B, dtype=tf.float32)
        # Weights
    ...
    def chebyshev5(self, x, L, Fout, K):
        ...
        L = tf.compat.v1.SparseTensor(indices, L.data, L.shape)
        L = tf.compat.v1.sparse_reorder(L)
        # Transform to Chebyshev basis
